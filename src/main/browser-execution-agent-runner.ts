...
export interface BrowserExecutionAgentRunnerOptions {
  ...
  onTraceEvent?: (event: BrowserExecutionTraceEvent) => void
}
...
export class BrowserExecutionAgentRunner {
  ...
  /** Ends the active run at its next loop boundary; confirmed steps are kept. */
  requestStop(): void {
  ...
  async run(instruction: string): Promise<BrowserExecutionAgentRunResult> {
    ...
    this.addTrace(
      trace,
      observationTrace('initial', initialObservation, budget.used)
    )
  ...
  private async execute(
    ...
    while (budget.tryUse()) {
      if (this.stopRequested) {
        return userStopped(budget.used, trace)
      }
      ...
      this.addTrace(trace, toolRequestTrace(toolCall, budget.used, target))
      ...
      this.addTrace(trace, toolResultTrace(toolCall, result, budget.used))
  ...
  private addTrace(
    trace: BrowserExecutionTraceEvent[],
    event: BrowserExecutionTraceEvent
  ): void {
    trace.push(event)
...
function userStopped(
  ...
  return { status: 'stopped', reason: 'user_requested', callsUsed, trace }
}
