...
describe('PlaywrightTestController', () => {
  ...
  it('writes the session title into the generated spec name', async () => {
    const writeSpec = vi.fn(async () => undefined)
    const controller = new PlaywrightTestController({
      currentDirectory: 'C:/TestGen',
      createRunDirectory: async () => 'C:/TestGen/testgen-playwright-run-c',
      removeDirectory: async () => undefined,
      runSpec: async () => ({ status: 'passed' as const, output: '1 passed' }),
      writeSpec
    })

    await controller.run({ steps: [] }, 'headless', 'protocol search')

    expect(writeSpec).toHaveBeenCalledWith(
      'C:/TestGen/testgen-playwright-run-c/live-test-progress.spec.ts',
      expect.stringContaining('test("protocol search", async ({ page }) => {')
    )
  })
})
