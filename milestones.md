...
intermediary for confirmed test facts. Live Test Progress shows an editable
in-session output and downloads an initial Excel-compatible CSV, without
becoming an execution queue or agent plan. The workspace UI follows the
Overhaul v4 design: one top bar with an editable session title and a single
lifecycle chip, a Setup tab for URL entry and the preconfigured one-click
login, a live chat trace of each agent run with a Stop brake, and the test
document absorbed into the workspace code panel (the separate Live Test
Progress window and the browser chrome placeholders are removed).
...
- Live Test Progress appends confirmed actions across submitted tasks to one
  editable in-session document shown in the workspace code panel. Its expanded
  document view (spec primary, CSV behind a toggle) can download the current
  rows as Excel-compatible CSV or a Playwright TypeScript spec named after the
  session title, including real entered values, and can run the current export
  in a separate Chromium process without affecting the embedded browser.
- **Restart** in the top bar clears the local authoring view and shared
  in-session document while preserving the embedded browser page; **Stop**
  ends an active agent run at its next loop boundary and keeps confirmed
  steps; a failed browser session offers an in-pane **Restart browser** that
  recreates the browser workspace without touching the document.
