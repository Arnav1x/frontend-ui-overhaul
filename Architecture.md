...
- A browser task completes only when the Browser Execution Agent returns a
  ...
  can adapt; it provides no queue or recovery subsystem. M06 started with a
  provisional 30-call combined budget; the default is now a 1000-call runaway
  backstop, with the workspace Stop action as the practical brake.
...
Live Test Progress is the workspace code panel, not a separate window: a
collapsed strip under the browser pane previews the latest captured lines, and
expanding it in place opens the full document view. The Playwright spec view is
primary and the CSV table stays live-viewable behind a small spec ⇄ csv
toggle. A row is appended only after a browser action succeeds and its
intermediary capture facts are available. It may be downloaded while a task
runs; it does not persist the document, execute edited rows, or treat
requested and failed actions as test facts. The session title in the top bar
becomes the exported spec's `test('…')` name. Credential-like fill values
render masked in views; exports keep entered values.

From the document view, the author can run the current rows through
`@playwright/test` as an early check in a separate Chromium. TestGen writes
the generated spec to a unique, ignored temporary directory under the
repository, starts one separate single-worker Playwright run, displays its
bounded output, then removes the temporary directory. The action does not
execute edited rows in the embedded browser and does not alter the in-session
document.

Restart is a main-workspace authoring action. It clears the local authoring
transcript and the shared in-session test document while preserving the
embedded browser page. It is unavailable during an active or
clarification-paused Browser Execution Agent task. Stop is the user-facing
brake on an active task: it asks the runner to end the run at its next loop
boundary, keeps the steps confirmed before the stop, and reports the stop as a
`user_requested` stopped result. The default call limit is a 1000-call runaway
backstop with no budget UI; Stop is the practical brake.

The main workspace also streams each Browser Execution Agent trace event to
the renderer while a run is active, so the chat rail can show the full
observe–act loop live; the terminal chat summary then collapses to durable
steps only. Browser session status (`starting` / `failed` / `closed`) is shown
inside the browser pane itself — the native browser view is parked off-screen
whenever it should not cover the workspace DOM — and a failed session offers a
pane-level browser restart that recreates the browser workspace without
touching the in-session document.
